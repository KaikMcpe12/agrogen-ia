...
export function Modal02NewAnimalStep2({ open, onClose, onBack, step1Data, mode = "create", animal }: Props) {
  useScrollLock(open);
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<Step2Data>({ mode: 'onBlur', reValidateMode: 'onChange',
    ...
    defaultValues: isEdit && animal?.dados_geneticos
      ? {
          raca_principal: animal.dados_geneticos.raca_principal,
          raca_pai: animal.dados_geneticos.raca_pai ?? "",
          raca_mae: animal.dados_geneticos.raca_mae ?? "",
          dep_peso_desmame: animal.dados_geneticos.dep_peso_desmame,
        }
      : {},
  });
  ...
  const mutation = useMutation({
    mutationFn: (data: Step2Data) => {
      ...
      if (isEdit && animal) {
        return animaisApi.atualizar(animal.id, payload);
      }
  ...
  const defaultRaca = step1Data?.especie === "BOVINO" ? "Nelore" : step1Data?.especie === "OVINO" ? "Dorper" : "Boer";
  const onSkip = () => mutation.mutate({ raca_principal: defaultRaca });
  const onSubmit = (data: Step2Data) => mutation.mutate(data);
      ...
      <div className="relative bg-surface flex flex-col w-full h-full md:h-auto md:max-w-lg md:rounded-[16px] md:border md:border-line md:max-h-[90dvh] md:shadow-[var(--shadow-lg)] md:mx-4">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-line shrink-0">
          <div>
            ...
            <h3 className="text-[17px] font-semibold text-ink" style={{ fontFamily: "var(--font-display)" }}>
              {isEdit ? `Editar Animal — ${animal?.codigo ?? ""}` : "Novo Animal — Dados Genéticos"}
            </h3>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
          <p className="text-[13px] text-ink-3">
            {isEdit ? "Atualize os dados genéticos do animal." : "Campos opcionais — pode pular se não tiver as informações genéticas agora."}
          </p>
        ...
        <div className="flex justify-between px-5 py-3 bg-beige border-t border-line shrink-0">
          <div className="flex gap-2">
            ...
            {!isEdit && (
              <Button variant="secondary" size="sm" onClick={onSkip} loading={mutation.isPending}>
                Salvar e Pular
              </Button>
            )}
          </div>
          <Button variant="primary" size="sm" onClick={handleSubmit(onSubmit)} loading={mutation.isPending}>
            {isEdit ? "Salvar Alterações" : "Salvar Animal"}
          </Button>
        </div>
      </div>
    </div>
