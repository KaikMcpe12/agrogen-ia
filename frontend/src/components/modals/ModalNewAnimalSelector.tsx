...
import { Modal01NewAnimalStep1, type Step1Data } from "./Modal01NewAnimalStep1";
import { Modal02NewAnimalStep2 } from "./Modal02NewAnimalStep2";
import { ModalChatNewAnimal } from "./ModalChatNewAnimal";

type Mode = "selector" | "tradicional_step1" | "tradicional_step2" | "chat";
...
export function ModalNewAnimalSelector({ open, onClose }: Props) {
  useScrollLock(open);
  ...
  // Delegate to traditional modals
  if (mode === "tradicional_step1") {
    return (
      <Modal01NewAnimalStep1
        ...
        onNext={(data) => {
          setStep1Data(data);
          setMode("tradicional_step2");
        }}
      />
    );
  }

  if (mode === "tradicional_step2") {
    return (
      <Modal02NewAnimalStep2
        open
        onClose={handleClose}
        onBack={() => setMode("tradicional_step1")}
        step1Data={step1Data}
      />
    );
  }
  ...
  if (mode === "chat") {
    return <ModalChatNewAnimal open onClose={handleClose} />;
  }
      ...
      <div className="relative bg-surface flex flex-col w-full h-full md:h-auto md:max-w-md md:rounded-[18px] md:border md:border-line md:shadow-[var(--shadow-lg)] md:mx-4">
        ...
        <div className="flex flex-col gap-4 px-5 py-6">
          <p className="text-[14px] text-ink-3">
            Como deseja registrar o animal?
          </p>

          {/* Tradicional */}
          <button
            onClick={() => setMode("tradicional_step1")}
          ...
          >
            ...
            <div>
              ...
              <p className="text-[13px] text-ink-3 mt-0.5 leading-relaxed">
                Preencha os campos em formulários organizados por etapa.
              </p>
            </div>
          </button>
