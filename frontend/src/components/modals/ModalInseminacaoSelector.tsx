...
interface Props {
  ...
  preselectedAnimalNome?: string;
}

export function ModalNewInseminacaoSelector({ open, onClose, preselectedAnimalId, preselectedAnimalNome }: Props) {
  useScrollLock(open);
  ...
  if (mode === "chat") {
    return (
      <ModalChatNewInseminacao
        open
        onClose={handleClose}
        preselectedAnimalId={preselectedAnimalId}
        preselectedAnimalNome={preselectedAnimalNome}
      />
    );
  }
