...
export function DiarioContent({ animalId }: DiarioContentProps) {
  ...
  const invalidateCurrent = () => {
    const key = tab === "peso" ? "pesagens" : tab === "paricao" ? "partos" : tab === "sanitario" ? "sanitario" : "ocorrencias";
  ...
  const tabs: { id: Tab; label: string; count?: number | undefined }[] = [
    ...
    { id: "ocorrencias", label: "Ocorrências", count: ocorrencias.filter((o) => !o.resolvida).length || undefined },
  ];
  ...
  return (
    <>
      ...
      <div className="flex border-b border-line">
        {tabs.map(({ id, label, count }) => (
          ...
          >
            {label}
            {count !== undefined && (
              <span className="ml-1.5 text-[11px] bg-green-100 text-green-900 font-semibold px-1.5 py-0.5 rounded-full">
                {count}
              </span>
            )}
          </button>
            ...
            <div className="flex gap-4 text-[13px] text-ink-3">
              {pesagensData?.resumo && (
                <>
                  ...
                  <span>GMD: <strong className="text-ink">{pesagensData.resumo.gmd_periodo !== undefined ? `${pesagensData.resumo.gmd_periodo} kg/dia` : "—"}</strong></span>
                  <span>Registros: <strong className="text-ink">{pesagensData.resumo.total_registros}</strong></span>
                </>
              )}
            </div>
            <Button variant="primary" size="sm" onClick={() => setModal06Open(true)}>+ Registrar Pesagem</Button>
          </div>
            ...
            <Card padding="sm">
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={chartData} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
                  ...
                  />
                  <Area type="monotone" dataKey="peso" stroke={chartTheme.primaryColor} strokeWidth={2} fill="url(#pesoGrad)" />
                </AreaChart>
              </ResponsiveContainer>
            </Card>
          ) : null}
      ...
      {tab === "paricao" && (
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-center">
            ...
            <Button variant="primary" size="sm" onClick={() => setModal07Open(true)} className="ml-auto">+ Registrar Parto</Button>
          </div>
          {parLoading ? (
            ...
            <div className="overflow-hidden rounded-[14px] border border-line bg-surface">
              <table className="w-full">
                <thead>
                  <tr className="bg-beige border-b border-line">
                    {["Data do Parto", "Tipo", "Crías", "Crías Vivas", "Distócia", "Óbito Matriz"].map((h) => (
                      <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-ink-2 uppercase tracking-[0.04em]">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {partos.map((p) => (
                    <tr key={p.id} className="border-b border-line last:border-0 hover:bg-beige/50 transition-colors">
                      <td className="px-4 py-2.5 text-[13px] text-ink">{new Date(p.data_parto).toLocaleDateString("pt-BR")}</td>
                      <td className="px-4 py-2.5 text-[12px] text-ink-2">{TIPO_PARTO_LABELS[p.tipo_parto] ?? p.tipo_parto}</td>
                      ...
                      <td className="px-4 py-2.5">
                        <Badge variant={p.houve_distorcia ? "danger" : "ok"}>{p.houve_distorcia ? "Sim" : "Não"}</Badge>
                      </td>
                      <td className="px-4 py-2.5">
                        <Badge variant={p.houve_obito_matriz ? "danger" : "ok"}>{p.houve_obito_matriz ? "Sim" : "Não"}</Badge>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {tab === "sanitario" && (
        <div className="flex flex-col gap-4">
          <div className="flex justify-end">
            <Button variant="primary" size="sm" onClick={() => setModal08Open(true)}>+ Registrar Evento Sanitário</Button>
          </div>
          {sanLoading ? (
            <div className="h-32 animate-pulse bg-beige rounded-[12px]" />
          ) : sanitario.length > 0 ? (
                ...
                <tbody>
                  {sanitario.map((s) => (
                    <tr key={s.id} className="border-b border-line last:border-0 hover:bg-beige/50 transition-colors">
                      ...
                      <td className="px-4 py-2.5 text-[13px] text-ink-2">{new Date(s.data_aplicacao).toLocaleDateString("pt-BR")}</td>
                      <td className="px-4 py-2.5 text-[12px] text-ink-3">{s.via ?? "—"}</td>
                      <td className="px-4 py-2.5 text-[12px] text-ink-2">
                        {s.proxima_dose ? new Date(s.proxima_dose).toLocaleDateString("pt-BR") : "—"}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <p className="text-center text-[14px] text-ink-3 py-8">Nenhum evento sanitário registrado.</p>
          )}
        </div>
      )}
      ...
      {tab === "ocorrencias" && (
        <div className="flex flex-col gap-4">
          <div className="flex justify-end">
            <Button variant="primary" size="sm" onClick={() => setModal09Open(true)}>+ Registrar Ocorrência</Button>
          </div>
          {ocoLoading ? (
            <div className="h-32 animate-pulse bg-beige rounded-[12px]" />
          ) : ocorrencias.length > 0 ? (
            <div className="flex flex-col gap-2">
              {[...ocorrencias].sort((a, b) => (a.resolvida === b.resolvida ? 0 : a.resolvida ? 1 : -1)).map((o) => (
                  ...
                  <div className="flex items-start gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant={o.resolvida ? "ok" : "warn"}>{o.resolvida ? "Resolvida" : "Em aberto"}</Badge>
                      </div>
                      <p className="text-[14px] font-semibold text-ink mt-1">{o.titulo}</p>
                      <p className="text-[13px] text-ink-3 mt-0.5">{o.descricao}</p>
                      <p className="text-[11px] text-ink-4 mt-1">{new Date(o.data).toLocaleDateString("pt-BR")}</p>
                    </div>
                  </div>
                </Card>
              ))}
      ...
      {!selectedAnimal ? (
        <div className="max-w-lg">
          ...
          {search.length > 1 && sugestoes.length === 0 && (
            <p className="text-[13px] text-ink-4 mt-2 px-1">Nenhum animal encontrado para "{search}".</p>
          )}
        </div>
      ) : (
        <>
          ...
          <Card padding="sm" className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center text-green-900 font-bold text-[15px]">
              {selectedAnimal.nome.charAt(0)}
            </div>
            <div className="flex-1">
              <p className="text-[16px] font-bold text-ink">{selectedAnimal.nome}</p>
              <p className="text-[12px] text-ink-3 font-mono">{selectedAnimal.codigo} · {selectedAnimal.especie} · {selectedAnimal.raca_principal}</p>
            </div>
            <Button variant="secondary" size="sm" onClick={() => setSelectedAnimal(null)}>Trocar animal</Button>
          </Card>
