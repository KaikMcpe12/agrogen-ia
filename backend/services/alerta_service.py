...
class AlertaService:
    ...
    async def get_by_id(self, alerta_id: UUID) -> dict:
        obj = await self.repo.get_by_id_with_animal(alerta_id)
        if not obj:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Alerta não encontrado.")
        return obj

    async def marcar_lido(self, alerta_id: UUID) -> dict:
        obj = await self.repo.marcar_lido(alerta_id)
        if not obj:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Alerta não encontrado.")
        return await self.repo.get_by_id_with_animal(alerta_id)

    async def marcar_resolvido(self, alerta_id: UUID) -> dict:
