...
```
score = clamp(prob_base_espécie + Σ deltas, 0.0, 1.0)
```

Onde `prob_base` é a taxa histórica por espécie (BOVINO 60%, OVINO/CAPRINO 65%) e os deltas são ajustes aditivos parametrizados por variável (condição corporal, intervalo pós-parto, ciclos sem concepção, DEP, temperatura, etc.). O resultado é truncado no intervalo `[0.0, 1.0]`, conforme a Matriz de IA (documento normativo).
